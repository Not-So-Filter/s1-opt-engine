...
RomEndLoc:	dc.l EndOfRom-1		; End address of ROM
		dc.l v_startofram	; Start address of RAM
		dc.l v_endofram		; End address of RAM
		if EnableSRAM=1
...
GameProgram:
		tst.w	(vdp_control_port).l	; wait for dma
		btst	#6,($A1000D).l
		beq.s	CheckSumOk
...
ptr_VBla_08:	dc.w VBla_08
ptr_VBla_0A:	dc.w VBla_0A
ptr_VBla_0C:	dc.w VBla_0C
ptr_VBla_10:	dc.w VBla_10
ptr_VBla_12:	dc.w VBla_12
ptr_VBla_14:	dc.w VBla_14
ptr_VBla_16:	dc.w VBla_16
; ===========================================================================
...
.waterbelow:
		...
		writeVRAM_noA5	v_hscrolltablebuffer,$380,vram_hscroll
		writeVRAM_noA5	v_spritetablebuffer,$280,vram_sprites
...
Tit_LoadText:
	...
	if debugbuild
		...
		move.b	d0,(f_nobgscroll).w ; reset nobgscroll flag
		bsr.w	LevelSizeLoad
		bsr.w	DeformLayers
		...
		copyTilemap	v_startofram,$C208,$21,$15

		lea	(Kos_GHZ_1st).l,a0 ; load GHZ patterns
		lea	(v_startofram).l,a1
...
LevelSelect:
		...
		moveq	#0,d0
		move.b	(v_levselitem).w,d0
		cmpi.b	#$14,d0		; have you selected item $14 (sound test)?
		bne.s	LevSel_Level_SS	; if not, go to	Level/SS subroutine
		move.b	(v_levselsound).w,d0
		tst.b	(f_creditscheat).w ; is Japanese Credits cheat on?
		beq.s	LevSel_PlaySnd	; if not, branch
		cmpi.b	#$9F,d0		; is sound $9F being played?
		beq.s	LevSel_Ending	; if yes, branch
		cmpi.b	#$9E,d0		; is sound $9E being played?
...
LevSel_Level_SS:
		...
		moveq	#0,d0
		move.w	d0,(v_zone).w	; clear	level
...
; ---------------------------------------------------------------------------
LevSel_Ptrs:	; correct level order
		dc.b id_GHZ, 0
		dc.b id_GHZ, 1
		dc.b id_GHZ, 2
		dc.b id_MZ, 0
...
LevSelControls:
		moveq	#btnUp+btnDn,d1	; is up/down pressed and held?
		and.b	(v_jpadpress1).w,d1
...
LevSel_UpDown:
		...
		moveq	#btnUp+btnDn,d1	; is up/down pressed?
		and.b	(v_jpadhold1).w,d1
...
LevSel_Refresh:
		move.b	d0,(v_levselitem).w ; set new selection
...
LevSel_SndTest:
		cmpi.b	#$14,(v_levselitem).w ; is item $14 selected?
		bne.s	LevSel_NoMove	; if not, branch
		moveq	#btnR+btnL,d1	; is left/right	pressed?
		and.b	(v_jpadpress1).w,d1
...
LevSel_Refresh2:
		move.b	d0,(v_levselsound).w ; set sound test number
...
Level_ChkDebug:
	if debugbuild
		...
		beq.s	Level_ChkWater	; if not, branch
		st.b	(f_debugmode).w ; enable debug mode
	endif
...
Level_ChkWater:
		...
		moveq	#id_WaterSurface,d0
		move.b	d0,(v_watersurface1).w ; load water surface object
		move.w	#$60,(v_watersurface1+obX).w
		move.b	d0,(v_watersurface2).w
...
Level_FDLoop:
		...
		jsr	(BuildSprites).l
		jsr	(ObjPosLoad).l
		subq.b	#1,(v_palchgspeed).w
		bpl.s	loc_3BC8
		move.b	#2,(v_palchgspeed).w
...
SS_FinLoop:
		move.b	#id_VBla_16,(v_vbla_routine).w
		bsr.w	WaitForVBla
		bsr.w	MoveSonicInDemo
		move.w	(v_jpadhold1).w,(v_jpadhold2).w
		jsr	(ExecuteObjects).l
...
loc_47D4:
		...
		bsr.w	PlaySound	 ; play end-of-level music
...
loc_491C:
		...
		move.w	#$4000,d0
		bsr.w	EniDec
		copyTilemap	v_startofram,$C000,$3F,$1F
		copyTilemap_end	v_startofram,$D000,$3F,$3F
...
Cont_ClrObjRam:
		...
		move.w	#$80*3,(v_continuelight+obPriority).w
...
End_LoadSonic:
		...
		st.b	(f_lockctrl).w ; lock controls
...
End_MoveSonic:
		...
		st.b	(f_lockctrl).w ; lock player's controls
...
LoadTilesAsYouMove_BGOnly:
		lea	(vdp_data_port).l,a6
		lea	4(a6),a5
; because only green hill zone does background only scrolling, there is no point to
; scrolling the first block because it doesn't exist in green hill zone's background
; deformation ~ Filter
...
;		bsr.w	DrawBGScrollBlock1
		...
		; why this is never included in background only is beyond me ~ Filter
		lea	(v_bg3_scroll_flags).w,a2	; Scroll block 3 scroll flags
		lea	(v_bg3screenposx).w,a3	; Scroll block 3 X coordinate
...
LevelDataLoad:
		...
		move.l	a2,-(sp)
		movea.l	(a2)+,a0
		lea	(v_startofram).l,a1 ; RAM address for art
		bsr.w	KosPlusDecVRAM
		movea.l	(a2)+,a0
...
NullObject:
		include	"_incObj/sub DeleteObject.asm" ; moved this to right after object pointers so that null objects go straight to deleting ~ Filter
		include	"_incObj/sub ObjectFall.asm"
		include	"_incObj/sub SpeedToPos.asm"
		include	"_incObj/sub DisplaySprite.asm"

		include	"_incObj/sub HUD.asm"
Map_HUD:	include	"_maps/HUD.asm"

; ===========================================================================
BldSpr_ScrPos:	dc.l 0				; blank
		dc.l v_screenposx	; main screen x-position
		dc.l v_bgscreenposx	; background x-position	1
		dc.l v_bg3screenposx	; background x-position	2
; ---------------------------------------------------------------------------
...
;		tst.b	(a0)		; if null, branch / s3k gets rid of this, because there should never be a moment where you load an object that doesn't exist ~ Filter
;		beq.w	.skipObject
...
loc_12C7E:
		...
		move.b	(v_anglebuffer+2).w,objoff_37(a0)
...
loc_1504A:
		subi.w	#$B,d3		; fixed a bug where Sonic can clip 1 pixel into the left wall of level collision ~ Filter
		eori.w	#$F,d3
		lea	(v_anglebuffer).w,a4
		movea.w	#-$10,a3
		move.w	#$400,d6	; MJ: $800/2
...
SS_AniRingSparks:
		...
		addq.b	#1,3(a0)
		movea.l	4(a0),a1
		move.b	SS_AniRingData(pc,d0.w),(a1)
		bne.s	locret_1B530
		clr.l	(a0)
		clr.l	4(a0)
...
SS_AniEmeraldSparks:
		...
		addq.b	#1,3(a0)
		movea.l	4(a0),a1
		move.b	SS_AniEmerData(pc,d0.w),(a1)
		bne.s	locret_1B60C
		clr.l	(a0)
		clr.l	4(a0)
...
SS_AniGlassBlock:
		...
		addq.b	#1,3(a0)
		movea.l	4(a0),a1
		move.b	SS_AniGlassData(pc,d0.w),(a1)
		bne.s	locret_1B640
		move.b	4(a0),(a1)
		clr.l	(a0)
		clr.l	4(a0)
...
SS_ClrRAM3:
		...
		lea	(v_startofram+$1020).l,a1
		lea	(v_startofram+$4000).l,a0
		moveq	#$3F,d1
...
Blk16_GHZ:	binclude	"map16/GHZ.eni"
		even
Kos_GHZ_1st:	binclude	"artkos/8x8 - GHZ1.kosp"	; GHZ primary patterns
		even
Kos_GHZ_2nd:	binclude	"artkos/8x8 - GHZ2.kosp"	; GHZ secondary patterns
		even
Blk128_GHZ:	binclude	"map128/GHZ.kosp"
		even
Blk16_LZ:	binclude	"map16/LZ.eni"
		even
Kos_LZ:		binclude	"artkos/8x8 - LZ.kosp"	; LZ primary patterns
		even
Blk128_LZ:	binclude	"map128/LZ.kosp"
		even
Blk16_MZ:	binclude	"map16/MZ.eni"
		even
Kos_MZ:		binclude	"artkos/8x8 - MZ.kosp"	; MZ primary patterns
		even
Blk128_MZ:	binclude	"map128/MZ.kosp"
		even
Blk16_SLZ:	binclude	"map16/SLZ.eni"
		even
Kos_SLZ:	binclude	"artkos/8x8 - SLZ.kosp"	; SLZ primary patterns
		even
Blk128_SLZ:	binclude	"map128/SLZ.kosp"
		even
Blk16_SYZ:	binclude	"map16/SYZ.eni"
		even
Kos_SYZ:	binclude	"artkos/8x8 - SYZ.kosp"	; SYZ primary patterns
		even
Blk128_SYZ:	binclude	"map128/SYZ.kosp"
		even
Blk16_SBZ:	binclude	"map16/SBZ.eni"
		even
Kos_SBZ:	binclude	"artkos/8x8 - SBZ.kosp"	; SBZ primary patterns
		even
